            ...
            <div class="mb-4">
                <form method="GET" action="{{ route('dashboard') }}">
                    ...
                    <input type="checkbox" id="hideCompleted" name="hideCompleted"
                           class="mr-2" {{ $hideCompleted ? 'checked' : '' }}>
                    <label for="search" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Search by
                        Task Title:</label>
                    <div class="flex mb-2">
                        <input type="text" id="search" name="search"
                               class="mt-1 p-2 border border-gray-300 dark:border-gray-700 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm rounded-md text-black"
                               placeholder="Enter task title..." value="{{ request()->get('search') }}">
                        <button type="submit"
                                class="ml-2 inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-800 transition duration-150 ease-in-out">
                            Search
                        </button>
                        <a href="{{ route('dashboard') }}"
                           class="ml-2 inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-gray-500 hover:bg-gray-400 focus:outline-none focus:border-gray-600 focus:shadow-outline-gray active:bg-gray-600 transition duration-150 ease-in-out">
                            Clear Filters
                        </a>
                    </div>

                    <div>
                        <label for="category" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter
                            by Category:</label>
                        <select id="category" name="category"
                                class="text-black mt-1 p-2 border border-gray-300 dark:border-gray-700 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm rounded-md">
