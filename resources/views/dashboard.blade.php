<x-app-layout>
    ...
    @if(session()->has('success'))
        <div
            class="flex justify-center bg-green-500 border border-green-700 text-white px-4 py-3 rounded relative max-w-lg mx-auto mt-6"
            role="alert">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex justify-center mb-6">
            <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                Your score: {{ $score }} Tasks completed: {{ $completedTasks->count() }} / {{ $totalTasksCount }}
                </div>
            </div>
        </div>
        ...
        <div class="text-gray-50 text-xl">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 px-4 sm:px-8 md:px-16 lg:px-24">
                @foreach($tasks as $task)
                    <div
                        class="py-8 sm:py-16 px-4 sm:px-8 flex flex-col justify-center h-full bg-zinc-900 shadow-sm sm:rounded-lg">
                        ...
                        <div class="flex items-center mt-2">
                            ...
                            <a href="{{ route('task', ['id' => $task->id]) }}"
                               class="bg-zinc-800 text-whit border border-red-400 ml-2 px-2 py-1 rounded hover:bg-red-400">Go
                                to Task</a>
                        </div>
                    </div>
                @endforeach
